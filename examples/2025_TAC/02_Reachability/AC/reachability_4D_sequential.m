...
r0 = x'*Dmax'*blkdiag(1/(4)^2, 1/(pi/30)^2, 1/(pi/15)^2, 1/(pi/30)^2)*Dmax*x - 2;
...
%% setup solver
sos1 = struct('x',[V;K;s2;s3;s4;s5;s6;s7;s8;s9;s10;b], ...  % dec.var
              'f',dot(g0-(V-b),g0-(V-b)), ...       % cost function for bisection
              'p',[]);                                  % parameter
...
Vval0 = subs(sol.x(1),t,0)-sol.x(end);
VvalT = subs(sol.x(1),t,T)-sol.x(end);
...
pcontour(subs(Vval,x(3:4),zeros(2,1)),0,[-1 1 -1 1]*2,'k--')

...
% 11/11/2008: PJS  Try to use integer sortrows_integerTable for speed
...
if  0
    try
        ...
        if nbits < 8
            [degsort,sortidx] = sortrows_integerTable( uint8(full(M)) );
        elseif nbits < 16
            [degsort,sortidx] = sortrows_integerTable( uint16(full(M)) );
        elseif nbits < 32
            [degsort,sortidx] = sortrows_integerTable( uint32(full(M)) );
        elseif nbits < 64
            [degsort,sortidx] = sortrows_integerTable( uint64(full(M)) );
        else

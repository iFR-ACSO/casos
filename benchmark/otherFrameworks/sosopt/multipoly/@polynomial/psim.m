...
%      (Default: nbig = 1e6, nsmall = 1e-6, xbig =inf, xsmall=0,
%          funchandle = [])
...
% TC  7/15/20  Instead of Boolean, return convergence flag indicating the
%              event that caused termination (positive: converged;
%              negative: diverged)
%              NOTE: To avoid breaking code, use |xconv > 0| to check for
%              convergence.
...
if isempty(event_params)
    ...
    CFlags = [];
elseif ~isempty(event_params.funchandle) && isempty(CFlags)
    [~,isterminal,~] = event_params.funchandle(0,x0mat(:,1),event_params);
    CFlags = zeros(1,length(isterminal));
    CFlags(isterminal) = -1;
end
opts.CFlags = [opts.CFlags CFlags];
...
for i1=1:N0
    ...
    if ~isempty(ie) %&& (ie(1)==2 || ie(1)==4)
        xconv(i1) = conv_index(opts.CFlags, ie(end));
    end
end
% xconv = logical(xconv);


function xconv = conv_index(CFlags, ie)

xconv = CFlags(ie)*sum(CFlags(1:ie)==CFlags(ie));

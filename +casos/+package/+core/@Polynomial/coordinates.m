function [c,S] = coordinates(obj,S)
% Return a vector of nonzero coordinates for a given basis (sparsity).
% If no basis is given, the vector's sparsity pattern is used.
...
elseif isempty(obj) || (isscalar(obj) && isempty(S)) || (islogical(S) && all(~S))
    ...
    if is_operator(obj)
        % empty operator
        S = casos.Sparsity.sparse_operator(0,0);
    else
        % empty polynomial
...
elseif islogical(S)
    ...
    assert(~is_operator(obj), 'Not supported.')
...
c = sparsity_cast(coeffs,casadi.Sparsity.dense(nnz(S),1));

...
V = casos.PS.sym('v',monomials([t;x],0:4));
k = casos.PS.sym('k',monomials([t;x],0:4));

% SOS multiplier
s1 = casos.PS.sym('s1',monomials([t;x;w],0:2),'gram'); % dissipation ineq.
s2 = casos.PS.sym('s2',monomials([t;x;w],2),'gram');  % dissipation ineq.

s3 = casos.PS.sym('s3',monomials([t;x],0:2),[ng 1],'gram');  % state constraint
s4 = casos.PS.sym('s4',monomials([t;x],2),[ng 1],'gram');  % state constraint

s51 = casos.PS.sym('s51',monomials([t;x;w],0:2),[nu 1],'gram'); % control constraint
s52 = casos.PS.sym('s52',monomials([t;x;w],0:2),[nu 1],'gram'); % control constraint

s61 = casos.PS.sym('s61',monomials([t;x;w],2),[nu 1],'gram');   % control constraint
s62 = casos.PS.sym('s62',monomials([t;x;w],2),[nu 1],'gram');   % control constraint

s7 = casos.PS.sym('s7',monomials(x,0:2),'gram');   % terminal set containment
s8 = casos.PS.sym('s8',monomials(x,0:2),'gram');   % grow constraint


s9  = casos.PS.sym('s9',monomials([t;x;w],2),'gram');   % terminal set containment
s10 = casos.PS.sym('s10',monomials([t;x;w],2),'gram');   % grow constraint
s11 = casos.PS.sym('s11',monomials([t;x;w],2),'gram');   % grow constraint
...
sos1.('g') = [s1*(V - g - R^2*q) - s2*hT - nabla(V,t) - nabla(V,x)*(f + gx*k)  + w'*w  +  s9*(w'*w - 2*R^2);
...
sos2.('g') = [s1sym*(V - g - R^2*q)  - s2sym*hT     - nabla(V-g,t) - nabla(V-g,x)*(f + gx*ksym) + w'*w   +  s9*(w'*w - 2*R^2) ;

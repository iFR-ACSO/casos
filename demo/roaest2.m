...
% solver 1: gamma-step
sos1 = struct('x',s1,'g',[ s1*(V-g)-nabla(V,x)*f-l],'p',[g;V]);
...
% solver 2: beta-step
sos2 = struct('x',s2,'g', s2*(p-b)+g-V,'p',[g;b;V]);
...
% s1 = casos.PS.sym()
Vlb = casos.PS(monomials(x,1:4),-inf(14,1));
Vub = casos.PS(monomials(x,1:4),+inf(14,1));

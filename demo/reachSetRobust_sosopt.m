...
s1  = sosdecvar('c1',monomials([t;x;w],0:2));
s2  = sosdecvar('c21',monomials([t;x;w],2));

s3 = sosdecvar('c3',monomials([t;x],0:2));
s4 = sosdecvar('c4',monomials([t;x],2));

s51  = sosdecvar('c51',monomials([t;x;w],0:2));
s52  = sosdecvar('c52',monomials([t;x;w],0:2));
s61  = sosdecvar('c61',monomials([t;x;w],2));
s62  = sosdecvar('c62',monomials([t;x;w],2));

s9   = sosdecvar('c9',monomials([t;x;w],2));
s10  = sosdecvar('c10',monomials([t;x;w],2));
s11  = sosdecvar('c11',monomials([t;x;w],2));
...
tic
 for iter = 1:10
    ...
    sosconk = [s1 >= 0;
               ...
               s10 >= 0;
               s11 >= 0;
               s8*(subs(Vval,t,t2)+gamma-R^2*q) - l >= 0;...
               subs(hU(1),u,k) + s51*(Vval+gamma-R^2*q) - s61*hT + s10*(w'*w - 2*R^2) >= 0;...
               subs(hU(2),u,k) + s52*(Vval+gamma-R^2*q) - s62*hT + s11*(w'*w - 2*R^2) >= 0;...
               s1*(Vval+gamma-R^2*q) - s2*hT - jacobian(Vval,t) - jacobian(Vval,x)*f - jacobian(Vval,x)*gd*k + w'*w  + s9*(w'*w - 2*R^2) >= 0;...
               s3*(Vval+gamma-R^2*q) - s4*hT - g >= 0
               ];
...
sosconV = [
           ...
           s10 >= 0;
           s11 >= 0;
           s1val*(V-gammaVal-R^2*q) - s2*hT - jacobian(V-gammaVal,t) - jacobian(V-gammaVal,x)*f - jacobian(V-gammaVal,x)*gd*kval + w'*w  + s9*(w'*w - 2*R^2) >= 0;...
           s3val*(V-gammaVal-R^2*q) - s4*hT - g >= 0;...
           subs(hU(1),u,kval) + s51val*(V-gammaVal-R^2*q) - s61*hT + s10*(w'*w - 2*R^2) >= 0;...
           subs(hU(2),u,kval) + s52val*(V-gammaVal-R^2*q) - s62*hT + s11*(w'*w - 2*R^2) >= 0;...
           s8val*(subs(V,t,t2)-gammaVal-R^2*q) - l >= 0
           s7*(subs(Vval,t,t1)-gammaVal-R^2*q)-(subs(V,t,t1)-gammaVal-R^2*q) >= 0
           ];

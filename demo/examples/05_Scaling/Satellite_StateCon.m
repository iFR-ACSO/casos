...
opts.Kx = struct('s', 1+length(s5));
opts.Kc = struct('s', 1+length(s5));
...
sos3       = struct('x',V, ...
                    'p',[s1_sym; s5_sym; s2_sym; g; b]);

sos3.('g') = [V - l;
              s1_sym*(V - g) - nabla(V,x)*f - l
              s2_sym*(p - b) + g - V;
              s5_sym*(V - g) - subs(gc,x,Dx^-1*x)];

opts    = struct;
opts.Kx = struct('s', 0, 'l', 1); 
opts.Kc = struct('s', 3+length(s5));

...
% SOS multiplier
s = casos.PS.sym('s',monomials(x,0:5),[length(g) 1],'gram');
r = casos.PS.sym('r',monomials(x,0:6),[length(l) 1],'gram');
...
% define SOS feasibility2
sos2 = struct('x',h, ...
              'f', int2( ( h-g(3))^2 ,x,[-3 3 -3 3]),...
              'g',[s_sym.*h.*ones(length(g),1) - g; ...
                   r_sym.*h.*ones(length(l),1) + l], ...
              'p',[s_sym;r_sym]);

% states + constraint are SOS cones
opts.Kx.l = 1; 
opts.Kx.s = 0;
opts.Kc.s = length(g)+length(l);
...
for iter = 1:itermax
   ...
   sol1 = S1('p',h_star);
    ...
    switch (S1.stats.UNIFIED_RETURN_STATUS)
     ...
     sol2 = S2('p',[sol1.x],'lbx',hlb,'ubx',hub);
    ...
    if iter < 2
     costold = sol2.f;
        ...
        if abs(double(costold-sol2.f)) < 1e-3
            disp('Cost function converged')
            break
        end
        costold = sol2.f;
    end
...
cleanfigure();
matlab2tikz('safesetCost.tex','width','\figW','height','\figH');

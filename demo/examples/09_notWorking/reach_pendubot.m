...
s51 = casos.PS.sym('s51',monomials([x;t],0:4));
s52 = casos.PS.sym('s52',monomials([x;t],0:4));

s61 = casos.PS.sym('s61',monomials([x;t],0:4));
s62 = casos.PS.sym('s62',monomials([x;t],0:4));
...
sos1.('g') = [s2;s1;s51;s52;s61;s62;s4-0.0001;
...
lbx = [casos.PS(basis(k),-inf);casos.PS(basis(s2),-inf);casos.PS(basis(s1),-inf);casos.PS(basis(s51),-inf);casos.PS(basis(s52),-inf);casos.PS(basis(s61),-inf);casos.PS(basis(s62),-inf);casos.PS(basis(s4),-inf)];
ubx = [casos.PS(basis(k),inf);casos.PS(basis(s2),inf);casos.PS(basis(s1),inf);casos.PS(basis(s51),inf);casos.PS(basis(s52),inf);casos.PS(basis(s61),inf);casos.PS(basis(s62),inf);casos.PS(basis(s4),inf)];
...
for iter = 1:1
    ...
    while (num_exp <= 12 || go)
        ...
        sol_Sstep = solve_Sstep('p',[Vval;gamma_try],'ubx',ubx,'lbx',lbx);
